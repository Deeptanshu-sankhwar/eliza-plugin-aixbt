...
# Task: Generate a post using ONLY recent data and current market information
Write a post that is {{adjective}} about {{topic}} (without mentioning {{topic}} directly), from the perspective of {{agentName}}. 

IMPORTANT GUIDELINES:
- Preferably focus on ONE topic/token only per thread - maintain topic consistency
- Use ONLY facts and data from the most recent provider updates
- Focus on current market trends and news from the last provider refresh
- Do not make up statistics or reference outdated information
- Only discuss tokens and news that were specifically mentioned in recent provider data
- If unsure about current data, focus on general market sentiment from recent updates
- Avoid speculation or historical references unless explicitly provided in recent data

Your response should be 1-5 sentences, optionally organized as a thread (2-3 posts) if the topic requires more depth. If writing a thread, separate posts with [THREAD_BREAK]. The total character count for each section MUST be less than {{maxTweetLength}}. No emojis. Use \\n\\n (double spaces) between statements if there are multiple statements in your response.

EXAMPLE THREADS:

Example 1 (3-post thread about one topic - USDT on Lightning):
"$USDT now live on Bitcoin Lightning Network. $140B supply, fresh $1B mint yesterday\\n\\nunstoppable tech meets unstoppable money."
[THREAD_BREAK]
"tether minting directly on lightning changes the game. no more wrapped tokens or intermediaries\\n\\npure bitcoin base layer settlement."
[THREAD_BREAK]
"major implications: instant $USDT transfers, near-zero fees, bitcoin ecosystem gets direct stablecoin liquidity without leaving the network."

Example 2 (2-post thread about one topic - LOFI):
"phantom wallet integration puts $LOFI in front of 15m+ daily active users. direct buy integration live\\n\\n$SUI ecosystem getting serious about distribution"
[THREAD_BREAK]
"market waking up to $LOFI accessibility play\\n\\n71% 24h move with $12.7m volume\\n\\nsitting at $82m mcap vs $250m previous peak"

Follow these examples for style, depth, and single-topic focus.`;
...
export class TwitterPostClient {
    ...
    async sendStandardTweet(
        ...
        try {
            ...
            if (body.errors) {
                elizaLogger.error("Error sending tweet:", body.errors);
                return null;
            }
            
            // Properly extract the tweet result
    ...
    async postTweet(
        ...
        try {
            elizaLogger.log(`Posting new tweet/thread:\n`);
            
            // Log the raw content being processed
            elizaLogger.log(`Raw content to process:\n${cleanedContent}`);

            // Split content into thread parts if [THREAD_BREAK] is present
            const threadParts = cleanedContent.split('[THREAD_BREAK]')
                .map(part => part.trim())
                .filter(part => part.length > 0);
            
            elizaLogger.log(`Found ${threadParts.length} parts in thread`);
            threadParts.forEach((part, i) => {
                elizaLogger.log(`Thread part ${i + 1}/${threadParts.length}:\n${part}`);
            });
            ...
            for (const [index, part] of threadParts.entries()) {
                elizaLogger.log(`Starting to post part ${index + 1}/${threadParts.length}`);
                elizaLogger.log(`Content: ${part}`);
                elizaLogger.log(`Previous tweet ID: ${previousTweetId || 'none'}`);
                
                let result;
                try {
                    if (part.length > DEFAULT_MAX_TWEET_LENGTH) {
                        elizaLogger.log(`Using NoteTweet for part ${index + 1} (length: ${part.length})`);
                        result = await this.handleNoteTweet(client, part, previousTweetId);
                    } else {
                        elizaLogger.log(`Using standard tweet for part ${index + 1} (length: ${part.length})`);
                        result = await this.sendStandardTweet(client, part, previousTweetId);
                    }

                    if (!result) {
                        elizaLogger.error(`Failed to post thread part ${index + 1} - null result received`);
                        continue;
                    }
                    ...
                    elizaLogger.log(`Successfully posted part ${index + 1}. Tweet ID: ${tweet.id}`);
                    elizaLogger.log(`Tweet URL: https://twitter.com/${twitterUsername}/status/${tweet.id}`);

                    // Add small delay between tweets
                    if (index < threadParts.length - 1) {
                        elizaLogger.log('Waiting 1 second before next tweet...');
                        await new Promise(resolve => setTimeout(resolve, 1000));
                    }
                } catch (error) {
                    elizaLogger.error(`Error posting thread part ${index + 1}:`, error);
            ...
            elizaLogger.log(`Thread posting completed. Posted ${tweets.length}/${threadParts.length} tweets`);

            // Process and cache the entire thread
            if (tweets.length > 0) {
                ...
                if (tweets.length > 1) {
                    await runtime.cacheManager.set(
                        `twitter/${twitterUsername}/thread/${mainTweet.id}`,
                        tweets.map(t => t.id)
                    );
                    elizaLogger.log(`Thread relationship cached for ${tweets.length} tweets`);
                }
            }
        } catch (error) {
            elizaLogger.error("Error in postTweet:", error);
            elizaLogger.error("Full error details:", JSON.stringify(error, null, 2));
        }
    }
    ...
    async generateNewTweet() {
        ...
        try {
            ...
            // Split into thread parts and validate each part's length
            const threadParts = cleanedContent.split('[THREAD_BREAK]')
                .map(part => part.trim())
                .filter(part => part.length > 0);

            elizaLogger.log(`Thread parts detected: ${threadParts.length}`);
            
            // Validate and truncate each part if necessary
            const maxTweetLength = this.client.twitterConfig.MAX_TWEET_LENGTH;
            const processedParts = threadParts.map(part => 
                maxTweetLength ? truncateToCompleteSentence(part, maxTweetLength) : part
            );

            // Rejoin the parts with [THREAD_BREAK]
            cleanedContent = processedParts.join('[THREAD_BREAK]');
            ...
            if (this.isDryRun) {
                elizaLogger.info(
                    `Dry run: would have posted thread:\n${cleanedContent}`
                );
                return;
            }
            ...
            try {
                if (this.approvalRequired) {
                    ...
                    elizaLogger.log(`Posting new thread:\n${cleanedContent}`);
